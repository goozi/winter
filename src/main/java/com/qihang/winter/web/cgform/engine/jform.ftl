...
<input id="preTitle" value="${preTitle}" type="hidden">
<input type="hidden" name="id" value="${id?if_exists?html}">
<input id="auditKey" value="${flowInfo}" type="hidden"/>
<div id="info" style="float: left; width: 80%">
</#if>
<form id="formobj" action="cgFormBuildController.do?saveOrUpdate<#if isAudit=='true'>&isAudit=true&auditKey=${flowInfo}</#if>" name="formobj" method="post">
<#if (head.windowType=='tab')>
    ...
    <#if isAudit=="true">
        <#if isAdd=='false'>
            <input type="button" id="sub_btn" class="button" value="提交"/>
        </#if>
    </#if>
    </div>
    <br/><br/><br/>
...
<#list columnhidden as po>
    ...
    <#list columns as po>
        ...
        <td class="value">
            ...
            <span class="Validform_checktip">
                <#if po.field_valid_type?if_exists?html?starts_with('*')  || (po.field_valid_type?if_exists?html == '' && po.is_null != 'Y')>
                    *
                </#if>
						</span>
    ...
    <#list columnsarea as po>
            ...
            <tr>
                ...
                <td class="value" colspan="3">
						<textarea id="${po.field_name}" name="${po.field_name}"
                                  class="inputxtarea" rows="6" cols="${po.field_length}"
                            <#if po.operationCodesReadOnly?if_exists> readonly="readonly"</#if>
                            <#if po.field_valid_type?if_exists?html != ''>
                                  datatype="${po.field_valid_type?if_exists?html}"
                            <#else>
        ...
        <#else>
            ...
            <span class="Validform_checktip"><#if po.field_valid_type?if_exists?html?starts_with('*') || (po.field_valid_type?if_exists?html == '' && po.is_null != 'Y')>
                *
            </#if></span>
                    <label class="Validform_label" style="display: none;">${po.content?if_exists?html}</label>
                </td>
        ...
        <#if isTree == "true">
            ...
            $('#tree').tree({
                ...
                onClick:function(node){
                    ...
                    iframe.name = "auditInfo";
                    if(isRead){
                        url+="&mode=read";
                    }
                    //iframe.setAttribute("src","cgFormBuildController.do?ftlForm&tableName=t_test_user"+(isRead ? "&mode=read" : "")+"&isTree=false&id="+id+"&preTitle="+preTitle);
                    iframe.setAttribute("src",url+"&id="+id+"&preTitle="+preTitle);
                    //iframe.setAttribute("src","userController.do?user");
                    iframe.setAttribute("style","height:700px;width:100%;frameborder:'no';border:0");
...
//                    }
        ...
        $("#formobj").Validform({
            ...
            callback: function (data) {
                ...
                if (!neibuClickFlag) {
                <#if (head.windowType=='tab')>
                    var tab = window.top.$('#maintabs').tabs('getTab', '${preTitle}');
                    var win = $(tab).children(':first')[0].contentWindow;
                    win.reloadTable();
                <#else >
                    var win = frameElement.api.opener;
                    win.reloadTable();
...
<script type="text/javascript">
  var nodeid = '${parentid?if_exists?html}';
  var operation = '${operation?if_exists?html}';
    $(function () {
        ...
        <#if isAdd=='false'>
            $('#sub_btn').click(function(){
                document.formobj.action="cgFormBuildController.do?saveOrUpdate&isSubmit=true&isAudit=true&auditKey=${flowInfo}"
