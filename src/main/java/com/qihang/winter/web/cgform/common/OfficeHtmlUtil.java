package com.qihang.winter.web.cgform.common;
...
=======
   ...
   * @param htmlStr 要处理的HTML
   */
  public String doHtml(String htmlStr, String editorType) {
    java.util.regex.Pattern pattern;
    java.util.regex.Matcher matcher;
...
//      ls_include.append(StringUtil.replace("<script type=\"text/javascript\" src=\"plug-in/tools/curdtools_{0}.js\"></script>\n", "{0}", lang));
//
...
//      ls_include.append("<script type=\"text/javascript\" src=\"plug-in/Validform/plugin/passwordStrength/passwordStrength-min.js\"></script>\n");
      ...
      ls_include.append("<script type=\"text/javascript\">$(function(){$(\"#formobj\").Validform({tiptype:1,\n");
...
*/
        ...
        StringBuilder styleInclude = new StringBuilder("");
        styleInclude.append("<style>\n");
        styleInclude.append("body{font-size:12px;}\n");
        styleInclude.append("table{border: 1px solid #000000;padding:0; margin:0 auto;border-collapse: collapse;width:100%;align:right;}\n");
        styleInclude.append("td {border: 1px solid #000000;background: #fff;font-size:12px;padding: 3px 3px 3px 8px;color: #000000;word-break: keep-all;}\n");
        styleInclude.append("</style>\r\n<body\n");

        pattern = Pattern.compile("<body", Pattern.CASE_INSENSITIVE);
        matcher = pattern.matcher(htmlStr);
        htmlStr = matcher.replaceAll(styleInclude.toString().replace("$", "\\$"));
      }
      /*
>>>>>>> .r14628
			//添加input语句
...
=======
   ...
   * @return
   */
  public String doSubTable(String htmlStr, String cgformName) {
    org.jsoup.nodes.Document doc = Jsoup.parse(htmlStr);
    Elements mainData = doc.select("div p:not(table p)");
    for (Element element : mainData) {
      if (StringUtil.isNotEmpty(element.text())) {
        element.text(element.text().replaceAll("#\\{([a-zA-Z0-9_]*)\\}", "\\$\\{" + cgformName + "\\.$1\\}"));
      }
    }
    Elements tableElements = doc.select("table");
    for (int i = 0; i < tableElements.size(); i++) {
      Element tableElement = tableElements.get(i);
      String subTableStr = tableElement.select("tr:eq(0) td p").get(0).text();
      subTableStr = subTableStr.substring(2, subTableStr.length() - 1);
      String[] subTableNames = subTableStr.split("\\|");
      if (subTableNames[0].equals("subTable")) {
        String subTableName = subTableNames[1];
        Elements trs = tableElement.select("tr");
        Element headTr = trs.get(1);
        Element dataTr = trs.get(2);
//        Element subList = dataTr.wrap("<#list "+subTableName+" as sub ></#list>");
        Elements datas = dataTr.select("td p");
        for (Element data : datas) {
          String fieldName = data.text();
          fieldName = fieldName.substring(2, fieldName.length() - 1);
          if(fieldName.equals("index")){
            data.text("${sub_index}");
          }else {
            data.text("${sub." + fieldName + "}");
          }
        }
        trs.get(0).remove();//删除子表第一行
        dataTr.before("<#list " + subTableName + " as sub>");
        dataTr.after("&lt;/#list&gt;");
      } else {
        continue;
      }
