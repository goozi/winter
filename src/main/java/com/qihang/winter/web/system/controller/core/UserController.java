...
import com.qihang.winter.core.common.model.json.ValidForm;
...
import com.qihang.winter.tag.vo.datatable.DataTables;
...
public class
  ...
  public void menu(HttpServletRequest request, HttpServletResponse response) {
    ...
    List<TSFunction> smallActionlist = new ArrayList();// 二级权限菜单
   ...
   * @return
   */
  @RequestMapping(params = "user_oa")
  public String user_oa(HttpServletRequest request) {
    // 给部门查询条件中的下拉框准备数据
    List<TSDepart> departList = systemService.getList(TSDepart.class);
    request.setAttribute("departsReplace", RoletoJson.listToReplaceStr(departList, "departname", "id"));
    return "system/user/userList-oa";
  }
  ...
  public AjaxJson lock(String id, HttpServletRequest req) {
    ...
    if ("programmer".equals(user.getUserName())) {
      message = "超级管理员[programmer]不可操作";
      j.setMsg(message);
      return j;
    }
...
//			}
  ...
  public void datagrid(TSUser user, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
    ...
    TSUser loginUser = ResourceUtil.getSessionUserName();
    cq.isNull("comId");
    if(!"programmer".equals(loginUser.getUserName())) {
        //cq.not(Restrictions.in("userName",new Object[]{"admin","scadmin","programmer"}));
        cq.add(Restrictions.not(Restrictions.in("userName",new Object[]{"admin","scadmin","programmer"})));
        ...
        if (!CollectionUtils.isEmpty(orgIdList)) {
            ...
            Set<String> list = systemService.getAllSonId(depart.getId());
//          ("with departs as (select id from " +
//                    "t_s_depart where id = '" + depart.getId() + "' union all select t_s_depart.id" +
//                    " from departs,t_s_depart where departs.id = t_s_depart.parentdepartid)" +
//                    "select s.id from departs s ");
            CriteriaQuery subCq = new CriteriaQuery(TSUserOrg.class);
            subCq.setProjection(Property.forName("tsUser.id"));
...
//    dataGrid.setTotal(cfeList.size());
...
//            users.setTSDepart(user.getTSDepart());
      ...
      users.setSonCompanyId(user.getSonCompanyId());
      users.setProjectCode(user.getProjectCode());
      users.setReadType(user.getReadType());
      users.setIsMyself(user.getIsMyself());
      systemService.updateEntitie(users);
      ...
      if (StringUtil.isNotEmpty(roleid)) {
        if(1 == user.getIsMyself()){
          TSRole role = systemService.findUniqueByProperty(TSRole.class,"roleCode","sc_is_myself");
          if(roleid.indexOf(role.getId()) < 0){
            if(roleid.endsWith(",")) {
              roleid += role.getId() + ",";
            } else {
              roleid += ","+role.getId()+",";
          ...
          TSRole role = systemService.findUniqueByProperty(TSRole.class,"roleCode","sc_is_myself");
          roleid = roleid.replaceAll(","+role.getId(),"");
        }
        if(null == user.getReadType() || (null != user.getReadType() && 0 == user.getReadType())){
          TSRole role1 = systemService.findUniqueByProperty(TSRole.class,"roleCode","sc_info_one");
          TSRole role = systemService.findUniqueByProperty(TSRole.class,"roleCode","sc_info_more");
          roleid = roleid.replaceAll("," + role.getId(), "");
          if(roleid.indexOf(role1.getId()) < 0) {
            if(roleid.endsWith(",")) {
              roleid += role.getId() + ",";
            } else {
              roleid += ","+role.getId()+",";
        ...
        } else if(1 == user.getReadType()){
          TSRole role1 = systemService.findUniqueByProperty(TSRole.class,"roleCode","sc_info_one");
          TSRole role = systemService.findUniqueByProperty(TSRole.class,"roleCode","sc_info_more");
          roleid = roleid.replaceAll("," + role1.getId(),"");
          if(roleid.indexOf(role.getId()) < 0) {
            if(roleid.endsWith(",")) {
              roleid += role.getId() + ",";
            } else {
...
//				}
        ...
        if (StringUtil.isNotEmpty(roleid)) {
          if(null != user.getReadType() && 1 == user.getReadType()){
            TSRole role = systemService.findUniqueByProperty(TSRole.class,"roleCode","sc_info_more");
            if(roleid.endsWith(",")) {
              roleid += role.getId() + ",";
            } else {
            ...
            TSRole role = systemService.findUniqueByProperty(TSRole.class,"roleCode","sc_info_one");
            if(roleid.endsWith(",")) {
              roleid += role.getId() + ",";
            } else {
              roleid += ","+role.getId()+",";
          ...
          if(null != user.getIsMyself() && 1 == user.getIsMyself()){
            TSRole role = systemService.findUniqueByProperty(TSRole.class,"roleCode","sc_is_myself");
            if(roleid.endsWith(",")) {
              roleid += role.getId() + ",";
            } else {
  ...
  protected void saveRoleUser(TSUser user, String roleidstr) {
    ...
    for (int i = 0; i < roleids.length; i++) {
      ...
      if(StringUtil.isNotEmpty(roleids[i])) {
  ...
  public void datagridRole(TSRole tsRole, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
    ...
    tsRole.setProjectCode(ResourceUtil.getConfigByName("PROJECT_CODE"));
  ...
  public ModelAndView addorupdate(@Value("#{sysProp[PROJECT_CODE]}")String projectCode, TSUser user, String model, HttpServletRequest req) {
...
//    List<Object[]> comList = userService.loadComList(currLoginUser);
//    List<Map<String, Object>> params = new ArrayList<Map<String, Object>>();
//    for (int i = 0; i < comList.size(); i++) {
//      Object[] type = (Object[]) comList.get(i);
//      Map<String, Object> com = new HashMap<String, Object>();
//      com.put("id", type[0]);
//      com.put("name", type[1]);
//      params.add(com);
//    }
//    req.setAttribute("comList", params);
//    req.setAttribute("comSize", params.size());
    String departid = oConvertUtils.getString(req.getParameter("departid"));
    //如果不是管理员则默认不让选择其余部门
    if (!StringUtil.isEmpty(departid) || !currLoginUser.getUserName().equals("programmer")) {
      if (StringUtil.isEmpty(departid)) {
        departList.add((TSDepart) systemService.getEntity(TSDepart.class, currLoginUser.getCurrentDepart().getId()));
      } else {
    ...
    req.setAttribute("sonId",departid);
    List<String> orgIdList = new ArrayList<String>();
    if (StringUtil.isNotEmpty(user.getId())) {
      user = systemService.getEntity(TSUser.class, user.getId());

      //如果不是管理员则默认不让选择部门
//      if (!currLoginUser.getUserName().equals("programmer") && !"oa".equals(model)) {
//        orgIdList.add(currLoginUser.getCurrentDepart().getId());
//      } else {
...
//      }
      ...
      //如果新增页面则默认为当前登录人同部门
      orgIdList = systemService.findHql("select d.id from TSDepart d,TSUserOrg uo where d.id=uo.tsDepart.id and uo.tsUser.id=?", new String[]{currLoginUser.getId()});
    }
    idandname(req, user);
    //如果用户新增则自动赋值为当前工程配置的项目代码
    if(StringUtil.isEmpty(user.getId())){
      user.setProjectCode(projectCode);
    }
    req.setAttribute("user", user);
    req.setAttribute("orgIdList", JSON.toJSON(orgIdList));
    req.setAttribute("currDepartId", currLoginUser.getCurrentDepart().getId());
    req.setAttribute("currLoginUser",currLoginUser);
  ...
  public void idandname(HttpServletRequest req, TSUser user) {
    ...
    if (roleUsers.size() > 0) {
      for (TSRoleUser tRoleUser : roleUsers) {
        //如果当前登录用户为农资管理员则由他新建的用户角色均默认为农资监管用户
        if(null != tRoleUser.getTSRole()) {
          if ("农资管理员".equals(tRoleUser.getTSRole().getRoleName()) && StringUtil.isEmpty(user.getId())) {
            List<TSRole> tsRoleList = systemService.findByProperty(TSRole.class, "roleName", "农资监管用户");
            if (tsRoleList.size() > 0) {
              roleId = tsRoleList.get(0).getId();
              roleName = tsRoleList.get(0).getRoleName();
              break;
            }
          }
...
//			}
  ...
  @RequestMapping(params = "loadUserTree")
  @ResponseBody
  public List<Map<String,Object>>loadUserTree(){
    String departId = ResourceUtil.getSessionUserName().getCurrentDepart().getId();
    List<Map<String,Object>> result = userService.getUserTree(departId);
    return result;
  }
  ...
  public List<Map<String,Object>> userSel() {
    List<Map<String,Object>> list= userService.getOrgTreeList();
    return list;
  }
