...
public class IconController extends BaseController {
  ...
  public void datagrid(TSIcon icon, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
    ...
    cq.addOrder("iconName", SortDirection.asc);
  ...
  public AjaxJson saveOrUpdateIcon(HttpServletRequest request) throws Exception {
    ...
    // 图标的css样式
    String css = "";
    if (iconName.startsWith("sub")) {//二级菜单需加入坐标编移
      css = "." + icon.getIconClas() + "{background:url('../images/" + icon.getIconClas() + "." + icon.getExtend() + "') no-repeat 48px 8px}";
    } else {
      css = "." + icon.getIconClas() + "{background:url('../images/" + icon.getIconClas() + "." + icon.getExtend() + "') no-repeat}";
    }
    write(request, css);
