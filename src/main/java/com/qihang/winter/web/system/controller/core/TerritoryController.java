...
import com.qihang.winter.web.system.pojo.base.TSTerritory;
...
public class TerritoryController extends BaseController {
  ...
  public List<ComboTree> setPTerritory(HttpServletRequest request, ComboTree comboTree) {
    ...
    if (comboTree.getId() != null) {
      List<TSTerritory> inList = systemService.getListByCriteriaQuery(cqIn, false);
      comboTrees = systemService.ComboTree(territoryList, comboTreeModel, inList, true);
    } else {
      comboTrees = systemService.ComboTree(territoryList, comboTreeModel, null, true);
    }
    return comboTrees;
  }
