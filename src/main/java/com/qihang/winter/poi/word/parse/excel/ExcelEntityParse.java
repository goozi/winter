 ...
 */
package com.qihang.winter.poi.word.parse.excel;
 ...
 */
public class ExcelEntityParse extends com.qihang.winter.poi.excel.export.base.ExportBase {

    private static final Logger LOGGER = LoggerFactory.getLogger(ExcelEntityParse.class);

    private static void checkExcelParams(com.qihang.winter.poi.word.entity.params.ExcelListEntity entity) {
        if (entity.getList() == null || entity.getClazz() == null) {
            throw new com.qihang.winter.poi.exception.word.WordExportException(com.qihang.winter.poi.exception.word.enmus.WordExportEnum.EXCEL_PARAMS_ERROR);
        }

    }

    private int createCells(int index, Object t, List<com.qihang.winter.poi.excel.entity.params.ExcelExportEntity> excelParams,
    ...
    private Map<String, Integer> getTitleMap(XWPFTable table, int index, int headRows) {
        if (index < headRows) {
            throw new com.qihang.winter.poi.exception.word.WordExportException(com.qihang.winter.poi.exception.word.enmus.WordExportEnum.EXCEL_NO_HEAD);
        }
        ...
        for (int j = 0; j < headRows; j++) {
            ...
            for (int i = 0; i < cells.size(); i++) {
                ...
                if (StringUtils.isEmpty(text)) {
                    throw new com.qihang.winter.poi.exception.word.WordExportException(com.qihang.winter.poi.exception.word.enmus.WordExportEnum.EXCEL_HEAD_HAVA_NULL);
                }
                map.put(text, i);
            }
        }
     ...
     * @param listobj
     */
    public void parseNextRowAndAddRow(XWPFTable table, int index, com.qihang.winter.poi.word.entity.params.ExcelListEntity entity) {
        checkExcelParams(entity);
        // 获取表头数据
        Map<String, Integer> titlemap = getTitleMap(table, index, entity.getHeadRows());
        try {
            // 得到所有字段
            Field fileds[] = com.qihang.winter.poi.util.PoiPublicUtil.getClassFields(entity.getClazz());
            com.qihang.winter.poi.excel.annotation.ExcelTarget etarget = entity.getClazz().getAnnotation(com.qihang.winter.poi.excel.annotation.ExcelTarget.class);
