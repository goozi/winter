...
import com.qihang.winter.web.system.pojo.base.TSRoleFunction;
...
public class ResourceUtil {
   ...
   * @return
   */
  public static final String getDataSourceType(){
    HttpSession session = ContextHolderUtils.getSession();
    return session.getAttribute("dataSourceType").toString();
  }
...
//        requestPath = requestPath.substring(0, requestPath.indexOf("&", requestPath.indexOf("&") + 1));
//      }
//      int index = requestPath.indexOf("&clickFunctionId");
//      if(index>=0){
//        requestPath = requestPath.substring(0,index);
//      }
      requestPath = requestPath.substring(0,requestPath.indexOf("&"));
      if(StringUtil.isNotEmpty(request.getParameter("client"))){
        requestPath += "&client="+request.getParameter("client");
      }
      if(StringUtil.isEmpty(request.getParameter("client")) && StringUtil.isNotEmpty(request.getParameter("type"))){
        requestPath += "&type="+request.getParameter("type");
      }
    }
...
//				|| key.equals(DataBaseConstant.UPDATE_NAME)
    ...
    //替换为系统用户登录所使用的監管单位
    if (key.equals(DataBaseConstant.SYS_SON_ID) || key.equals(DataBaseConstant.SYS_SON_ID_TABLE)) {
      returnValue = getSessionUserName().getSonCompanyId();
    }
    //替换为系统用户登录所使用的主體单位
    if (key.equals(DataBaseConstant.SYS_COM_ID) || key.equals(DataBaseConstant.SYS_COM_ID_TABLE)) {
      returnValue = getSessionUserName().getComId();
    }
    //替换为当前系统时间(年月日)
