...
public class ListtoMenu {
   ...
   * @return 菜单字符串
   */
  public static String getBootstrapMenu(Map<Integer, List<TSFunction>> map, String style) {
    StringBuffer menuString = new StringBuffer();
    menuString.append("<ul class=\"nav navbar-nav\">");
    ...
    for (TSFunction pFunction : pFunctions) {
      ...
      menuString.append("			<span class=\"bootstrap-icon\" style=\"padding:0 9px;background: url('" + pFunction.getTSIcon().getIconPath()+ "') no-repeat;\"></span> " + getMutiLang(pFunction.getFunctionName()) + " ");
      if (hasSub) {
      ...
      //绘制二级菜单
      if (hasSub) {
        menuString.append(getBootStrapChild(pFunction, 1, map, style));
      }
      menuString.append("	</li> ");
    }
    menuString.append("</ul>");
   ...
   */
  private static String getBootStrapChild(TSFunction parent, int level, Map<Integer, List<TSFunction>> map, String style) {
    StringBuffer menuString = new StringBuffer();
    List<TSFunction> list = map.get(level);
    ...
    for (TSFunction function : list) {
      if (function.getTSFunction().getId().equals(parent.getId())) {
        ...
        if (style.equals("scm_bootstrap")) {
          menuString.append("		<li onclick=\"addTab(\'" + getMutiLang(function.getFunctionName()) + "\',\'" + menu_url + "\')\"  title=\"" + getMutiLang(function.getFunctionName()) + "\" url=\"" + function.getFunctionUrl() + "\" ");
        }
        if (hasSub) {
        ...
        menuString.append("			</a> ");
        if (hasSub) {
          menuString.append(getBootStrapChild(function, level + 1, map, style));
        }
        menuString.append("		</li> ");
      }
    }
...
//		data = dataString.substring(0, dataString.toString().length()-1);
  ...
  public static String getAceMultistageTree(Map<Integer, List<TSFunction>> map) {
    ...
    for (TSFunction function : list) {
      menuString.append("<li>");
      menuString.append("<a href=\"#\" class=\"dropdown-toggle\" ><i class=\"menu-icon " + function.getTSIcon().getIconClas() + "\"></i>")
              .append("<span class=\"menu-text\">" + getMutiLang(function.getFunctionName()) + "</span>");
      ...
      if (!function.hasSubFunction(map)) {
        ...
        menuString.append("</a><b class=\"arrow\"></b><ul  class=\"submenu\" >");
        menuString.append(getSubMenu(function, 1, map));
        menuString.append("</ul></li>");
      }
      menuString.append("<li class=\"qh-menu-divider\"></li>");
      curIndex++;
    }
    ...
    for (TSFunction function : list) {
      if (function.getTSFunction().getId().equals(parent.getId())) {
        if (!function.hasSubFunction(map)) {
          ...
          menuString.append("<li><a href=\"#\" class=\"dropdown-toggle\"><i class=\"menu-icon " +
                  function.getTSIcon().getIconClas() + "\" ></i>" + getMutiLang(function.getFunctionName()) + "</a><b class=\"arrow\"></b>");
  ...
  private static String getLeafOfACETree(TSFunction function) {
    ...
    menuString.append("<li iconCls=\"");
    menuString.append(icon + "\">");
//    menuString.append("<i class=\"menu-icon "+icon+"\"></i>");
    menuString.append("<a class=\"sub-menu-icon " + icon + "\" href=\"javascript:loadModule(\'");
