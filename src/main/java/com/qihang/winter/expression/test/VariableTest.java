package com.qihang.winter.expression.test;
...
import com.qihang.winter.expression.PreparedExpression;
...
//		System.out.println("Result = " + result);
		...
		//给表达式中的变量 [用户名] 付上下文的值
		List<Variable> variables = new ArrayList<Variable>();
		variables.add(new Variable("数字类型" , BaseDataMeta.DataType.DATATYPE_DOUBLE , new Integer(0)));
		//执行表达式
		PreparedExpression pe  = ExpressionEvaluator.preparedCompile(expression, variables);
