package com.qihang.winter.winterdao.util;
...
public class WinterDaoUtil {
  ...
  private static final Logger logger = Logger.getLogger(WinterDaoUtil.class);
  ...
  public static String createPageSql(String dbType, String sql, int page,
    ...
    if (jdbcType == null || "".equals(jdbcType)) {
      throw new RuntimeException(
              "WinterDaoHandler:(数据库类型:dbType)没有设置,请检查配置文件");
    }
  ...
  public static String getMethodSqlLogicJar(String sqlurl) {
    ...
    // 返回读取指定资源的输入流
    InputStream is = WinterDaoUtil.class.getResourceAsStream(sqlurl);
