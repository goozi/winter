...
<div style="width: auto;height: 300px;overflow-y:auto;overflow-x:scroll;">
  <table border="0" cellpadding="2" cellspacing="0" id="${entityName?uncap_first}_table">
    ...
    <#list columns as po>
        ...
        <#if check==0>
          <td align="left"
              bgcolor="#EEEEEE"><#if po.filedComment?length lt 7 >${po.filedComment}<#else>${po.filedComment[0..6]}</#if></td>
        </#if>
    </#list>
    </tr>
    ...
    <c:if test="${"$"}{fn:length(${entityName?uncap_first}List)  <= 0 }">
      ...
      <#list columns as po>
          ...
          <#if check==0>
            <td align="left"><input name="${entityName?uncap_first}List[0].${po.fieldName}"
    ...
    <c:if test="${"$"}{fn:length(${entityName?uncap_first}List)  > 0 }">
      <c:forEach items="${"$"}{${entityName?uncap_first}List}" var="poVal" varStatus="stuts">
        ...
        <#list columns as po>
            ...
            <#if check==0>
              <td align="left"><input name="${entityName?uncap_first}List[${'$'}{stuts.index }].${po.fieldName}"
                                      maxlength="${po.charmaxLength}" value="${'$'}{poVal.${po.fieldName} }" type="text"
