...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="div"
             action="${entityName?uncap_first}Controller.do?doUpdate" tiptype="1">
<#list columns as po>
    <#if po.isShow == 'N'>
          ...
          <label class="Validform_label">${po.content}:</label>
            ...
            <#elseif po.showType=='popup'>
              <input id="${po.fieldName}" name="${po.fieldName}" type="text" style="width: 150px"
                     class="searchbox-inputtext" value='${'$'}{${entityName?uncap_first}Page.${po.fieldName}}'
                     <#if po.fieldValidType?if_exists?html != ''>datatype="${po.fieldValidType?if_exists?html}" <#else><#if po.type == 'int'>datatype="n"
                     <#elseif po.type=='double'>datatype="/^(-?\d+)(\.\d+)?$/" <#else><#if po.isNull != 'Y'>datatype="*" </#if></#if></#if><#if po.dictTable?if_exists?html!="">onclick="inputClick(this,'${po.dictField}','${po.dictTable}')"</#if>
                     value='${'$'}{${entityName?uncap_first}Page.${po.fieldName}}'>
            ...
            <#elseif po.showType=='radio' || po.showType=='select' || po.showType=='checkbox' || po.showType=='list'>
              <t:dictSelect field="${po.fieldName}" type="${po.showType?if_exists?html}"
                            ...
                            <#else>typeGroupCode="${po.dictField}"</#if>
                            defaultVal="${'$'}{${entityName?uncap_first}Page.${po.fieldName}}" hasLabel="false"
            ...
            <#elseif po.showType=='file'>
              ...
              <c:if test="${'$'}{!empty ${entityName?uncap_first}Page.${po.fieldName}}">
                <a href="${'$'}{${entityName?uncap_first}Page.${po.fieldName}}" target="_blank"
                   id="${po.fieldName}_href" href='${'$'}{${entityName?uncap_first}Page.${po.fieldName}}'>下载</a>
              </c:if>
              <input class="ui-button" type="button" value="上传附件"
                     onclick="commonUpload(function(url,name){$(" #${po.fieldName}_href").attr('src',url).html('下载');$("#${po.fieldName}
              ").val(url);})"/>
            <#else>
              <input id="${po.fieldName}" name="${po.fieldName}" type="text" style="width: 150px" class="inputxt"
                  <#if po.fieldValidType?if_exists?html != ''>
