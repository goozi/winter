...
function resetTrNum(tableId) {
	...
	$tbody.find('tr').each(function(i){
		...
		$("input[name='tScRpRotherbillentryList["+i+"].findex']").val(i+1);
	});
}
//通用弹出式文件上传
...
function setAllAmount(obj){
	...
	var amount =  $("#tScRpRotherbillentry_table").find("input[name$='.amount']");
	var allAmount = 0;
...
function selectFeeDialog(index){
	var expName = $("input[name='tScRpRotherbillentryList["+index+"].expName']").val();
	...
	$.dialog({
		...
		button: [{
			...
			callback: function () {
				...
				//var nextTrLength = $('input[name="tScRpRotherbillentryList[' + index + '].findex"]').parent().nextAll().length;//判断后面的行数
				//var newLength = itemLength - nextTrLength - 1
				for (var j = 0; j < itemLength-1; j++) {
					clickAddTScRpRotherbillentryBtn(index);
				}
				$.each(expList, function (i, item) {
					var rowindex = parseInt(index) + i;
					var expId = item.id;
					var expName = item.name;
					$("input[name='tScRpRotherbillentryList["+rowindex+"].expId']").val(expId);
					$("input[name='tScRpRotherbillentryList["+rowindex+"].expName']").val(expName);
					setValOldIdAnOldName($("input[name='tScRpRotherbillentryList["+rowindex+"].expName']"),expId,expName);
				});
			},
			focus: true
		}, {
...
//判断不是选择而是手动写自动清空
function clearExp(obj){
	var expId = $(obj).parent().parent("tr").find("input[name$='.expId']").val();
