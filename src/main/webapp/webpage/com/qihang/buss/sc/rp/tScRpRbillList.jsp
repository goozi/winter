...
<div class="easyui-layout" fit="true">
    <div region="center" style="padding:1px;">
        <t:datagrid name="tScRpRbillList" checkbox="true" fitColumns="false" title="收款单" tranType="105" tableName="t_sc_rp_rbill" beforeAccount="true" scrollview="true" rowStyler="dgRowStyler"
                    actionUrl="tScRpRbillController.do?datagrid&isWarm=${isWarm}" idField="id" fit="true" queryMode="group" onDblClick="goDetail" isSon="true">
            ...
            <t:dgCol title="客户" field="itemName" mergeCells="true" query="true" queryMode="single" width="160"></t:dgCol>
            ...
            <t:dgCol title="收款类型" field="billType" mergeCells="true" dictionary="SC_BillType" queryMode="single" width="70"></t:dgCol>
            <t:dgCol title="收款金额" field="allAmount" extendParams="formatter:function(v,r,i){var m = $('#CFG_MONEY').val();var newV = parseFloat(v||0).toFixed(m); return newV;}," mergeCells="true" queryMode="single" width="70"></t:dgCol>
            ...
            <t:dgCol title="收款金额" field="amount" extendParams="formatter:function(v,r,i){var m = $('#CFG_MONEY').val();var newV = parseFloat(v||0).toFixed(m); return newV;}," queryMode="single" width="70"></t:dgCol>
            ...
            <t:dgCol title="作废标记" field="cancellation" hidden="true" mergeCells="true" query="false" replace="未作废_0,已作废_1" queryMode="single" width="65"></t:dgCol>
            ...
            <t:dgDelOpt title="删除" exp="checkState#eq#0&&cancellation#eq#0" url="tScRpRbillController.do?doDel&id={id}"/>
            <t:dgToolBar title="新增" icon="new-icon-add" url="tScRpRbillController.do?goAdd&tranType=105" funname="add"></t:dgToolBar>
            <t:dgToolBar title="编辑" icon="new-icon-edit" url="tScRpRbillController.do?goUpdate"
                         funname="goUpdate"></t:dgToolBar><!--update-->
            <t:dgToolBar title="复制" icon="new-icon-copy" url="tScRpRbillController.do?goUpdate"
                         funname="fcopy"></t:dgToolBar>
            <%--<t:dgToolBar title="批量删除"  icon="icon-remove" url="tScRpRbillController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>--%>
            ...
            <t:dgToolBar title="查看" icon="new-icon-view" url="tScRpRbillController.do?goUpdate"
            ...
            <t:dgToolBar title="打印" icon="new-icon-print"
                         funname="CreateFormPage('收款单', '#tScRpRbillList')"></t:dgToolBar>
        </t:datagrid>
    </div>
</div>
<script src="webpage/com/qihang/buss/sc/rp/tScRpRbillList.js"></script>
<script type="text/javascript">
    function goDetail(rowIndex, rowData){
        url = 'tScRpRbillController.do?goUpdate' + '&load=detail&id=' + rowData.id;
        createdetailwindow('查看', url, null, null,"tab");
    }
    $(document).ready(function () {
        //添加datagrid 事件 onDblClickRow
//        $("#tScRpRbillList").datagrid({
//
...
//        });
        //给时间控件加上样式
        $("#tScRpRbillListtb").find("input[name='createDate']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function () {
            WdatePicker({dateFmt: 'yyyy-MM-dd'});
        });
        $("#tScRpRbillListtb").find("input[name='updateDate']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function () {
            WdatePicker({dateFmt: 'yyyy-MM-dd'});
        });
        $("#tScRpRbillListtb").find("input[name='date_begin']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function () {
            WdatePicker({dateFmt: 'yyyy-MM-dd'});
        });
        $("#tScRpRbillListtb").find("input[name='date_end']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function () {
            WdatePicker({dateFmt: 'yyyy-MM-dd'});
        });
    });
    //编辑页面跳转
    function goUpdate(){
        var selected = $("#tScRpRbillList").datagrid("getSelected");
        if(null != selected) {
        ...
        var url = "tScRpRbillController.do?goUpdate";
        update("编辑", url, "tScRpRbillList", null, null, "tab");
    }
    //导入
    function ImportXls() {
        openuploadwin('Excel导入', 'tScRpRbillController.do?upload', "tScRpRbillList");
    }

    //导出
    function ExportXls() {
        JeecgExcelExport("tScRpRbillController.do?exportXls&tranType=105", "tScRpRbillList");
    }

    //模板下载
    function ExportXlsByT() {
        JeecgExcelExport("tScRpRbillController.do?exportXlsByT", "tScRpRbillList");
    }
</script>