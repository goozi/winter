...
; (function($) {
    ...
    if (!dateFormat || typeof (dateFormat) != "function") {
        var dateFormat = function(format) {
            var o = {
                ...
                "L": __MonthName[new Date(this).getMonth()] //non-standard
            };
            if (/(y+)/.test(format)) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        function CalDateShow(startday, endday, isshowtime, isshowweek) {
            if (!endday) {
                return dateFormat.call(startday, getymformat(new Date(startday), null, isshowtime));
            } else {
                var strstart = dateFormat.call(startday, getymformat(new Date(startday), null, isshowtime, isshowweek));
                var strend = dateFormat.call(endday, getymformat(new Date(endday), new Date(startday), isshowtime, isshowweek));
                var join = (strend != "" ? " - " : "");
        ...
        function parseED(data) {
            if (data.length > 6) {
                var e = [];
                e.push(data[0], data[1], new Date(parseInt(data[2],10)), new Date(parseInt(data[3],10)), parseInt(data[4],10), parseInt(data[5],10), parseInt(data[6],10), data[7] != undefined ? parseInt(data[7],10) : -1, data[8] != undefined ? parseInt(data[8],10) : 0, data[9], data[10]);
                return e;
            }
            return null;
        ...
        function dayshow(e, data) {
            ...
            if (data != null) {
                if (option.quickDeleteUrl != "" && data[8] == 1 && option.readonly != true) {
                    ...
                    ss.push(dateFormat.call(new Date(data[2]), i18n.xgcalendar.dateformat.Md3), " (", __WDAY[new Date(data[2]).getDay()], ")");
                    if (data[4] != 1) {
                        ss.push(",", dateFormat.call(new Date(data[2]),option.timeFormat));
                    }

                    if (iscos) {
                        ss.push(" - ", dateFormat.call(new Date(data[3]), i18n.xgcalendar.dateformat.Md3), " (", __WDAY[new Date(data[3]).getDay()], ")");
                        if (data[4] != 1) {
                    ...
                    $.post(option.quickUpdateUrl, param, function(data) {
                        if (data) {
                            if (data.IsSuccess == true) {
                                option.isloading = false;
                                option.onAfterRequestData && option.onAfterRequestData(5);
                            }
                            else {
                                option.onRequestDataError && option.onRequestDataError(5, data);
                                option.isloading = false;
                                //还原数据，重画										
                                d = rebyKey(id, true);
                                d[2] = os;
            ...
            if (buddle.length == 0) {
                ...
                calbutton.click(function(e) {
                    ...
                    var param = [{ "name": "CalendarTitle", value: what },
						{ "name": "startTime", value: datestart+":00" },
						{ "name": "endTime", value: dateend+":00" },
        ...
        function Ind(event, i) {
            ...
            if (!i) {
                if (option.eventItems && option.eventItems.length > 0) {
                    var sl = option.eventItems.length;
                    var s = event[2];
                    var d1 = s.getTime() - new Date(option.eventItems[0][2]).getTime();
                    var d2 = new Date(option.eventItems[sl - 1][2]).getTime() - s.getTime();
                    var diff = d1 - d2;
                    if (d1 < 0 || diff < 0) {
                        for (var j = 0; j < sl; j++) {
                            if (new Date(option.eventItems[j][2]) >= s) {
                                i = j;
                                break;
                            }
                        }
