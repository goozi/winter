...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$.ajax({
				type: "get",
				url: "tScRpRbillController.do?tScRpRbillentry1List&id=${tScRpRbillPage.id}&load=${load}",
			...
			$.ajax({
				type: "get",
				url: "tScRpRbillController.do?tScRpRbillentry2List&id=${tScRpRbillPage.id}&load=${load}",
				success:function(data){
					$("#southBody").html(data);
...
<t:formvalid formid="formobj" dialog="true" title="收款单" usePlugin="password" tranType="105" tableName="t_sc_rp_rbill" layout="table" tiptype="1" beforeSubmit="checkAllAmount"
			 action="tScRpRbillController.do?doUpdate" saveTemp="true">
	<div  class="panel-body layout-body" style="height: 20%">
		...
		<input id="tranType" name="tranType" type="hidden"
			   value="${tScRpRbillPage.tranType }">
		<input id="date" name="date" type="hidden"
			   value="${tScRpRbillPage.date }">
		<input id="billNo" name="billNo" type="hidden"
			   value="${tScRpRbillPage.billNo }">
		<input id="billerId" name="billerId" type="hidden"
			   value="${tScRpRbillPage.billerId }">
		<input id="checkerId" name="checkerId" type="hidden" value="${tScRpRbillPage.checkerId }">
		<input id="checkDate" name="checkDate" type="hidden" value="${tScRpRbillPage.checkDate }">
		<input id="checkState" name="checkState" type="hidden" value="${tScRpRbillPage.checkState }">
		<input id="cancellation" name="cancellation" type="hidden" value="${tScRpRbillPage.cancellation }">
		<input id="itemId" name="itemId" type="hidden" value="${tScRpRbillPage.itemId }">
		<input id="empId" name="empId" type="hidden" value="${tScRpRbillPage.empId }">
		<input id="deptId" name="deptId" type="hidden" value="${tScRpRbillPage.deptId }">
		<input id="sonId" name="sonId" type="hidden" value="${tScRpRbillPage.sonId }">
		<input id="billType" name="billType" type="hidden" value="${tScRpRbillPage.billType }">
			...
			<tr>
				...
				<td class="value">
					<t:dictSelect field="billType_" type="list" showDefaultOption="true"
								  ...
								  defaultVal="${tScRpRbillPage.billType }"
								  hasLabel="false"
      ...
      <span class="Validform_checktip">
				...
				<td class="value">
					<input id="allAmount" name="allAmount" readonly="readonly" type="text" style="width: 150px" class="inputxt"
						   value="${tScRpRbillPage.allAmount }"
							>
      <span class="Validform_checktip">
                </span>
      ...
      <span class="Validform_checktip">
			...
			<tr>
				...
				<td class="value" colspan="5">
					<input id="explanation" name="explanation" type="text" style="width: 47%" class="inputxt" value="${tScRpRbillPage.explanation }"
							>
      <span class="Validform_checktip">
                </span>
