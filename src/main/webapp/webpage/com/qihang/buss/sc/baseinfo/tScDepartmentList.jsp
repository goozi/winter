...
<div id="mm1" class="easyui-menu" style="width:120px;">
    <div onclick="addTree('06')" data-options="iconCls:'icon-add'"><font color="black" >添加</font></div>
...
<div class="easyui-layout" fit="true">
    <div data-options="region:'west',title:'部门分类',split:true" style="width:200px;">
        <ul id="tree" class="easyui-tree" data-options="url:'tScItemTypeController.do?getItemTypeTree&itemClassId=06'"></ul>
    </div>
    <div region="center" style="padding:1px;">
        <t:datagrid name="tScDepartmentList"  fitColumns="false" title="部门" isSon="false" tableName="t_sc_department"
                    actionUrl="tScDepartmentController.do?datagrid" idField="id" fit="true" colConfig="false"
            ...
            <t:dgCol title="助记码" field="shortNumber"    queryMode="single"   width="65"></t:dgCol>
            <t:dgCol title="部门主管" field="manager"  dictionary="t_sc_emp, id, name"  queryMode="single"   width="70"></t:dgCol>
            ...
            <t:dgCol title="启用信控" field="isCreditMgr"  replace="是_1,否_0"  queryMode="single"   width="65"></t:dgCol>
            <t:dgCol title="信用额度" field="creditAmount"    queryMode="single"   width="65"></t:dgCol>
            <t:dgCol title="禁用标记" field="disabled" replace="启用_0,禁用_1"   queryMode="single"   width="65"></t:dgCol>
            ...
            <t:dgDelOpt title="删除" exp="count#eq#0&&disabled#eq#0" url="tScDepartmentController.do?doDel&id={id}"/>
            <t:dgToolBar title="新增" icon="new-icon-add" url="tScDepartmentController.do?goAdd"
                         onclick="goAdd()"></t:dgToolBar>
            <t:dgToolBar title="编辑" icon="new-icon-edit" url="tScDepartmentController.do?goUpdate"
                         onclick="uploadEdit()"    funname="update"></t:dgToolBar>
            <%--<t:dgToolBar title="批量删除" icon="icon-remove" url="tScDepartmentController.do?doBatchDel"
                         funname="deleteALLSelect"></t:dgToolBar>--%>
            <t:dgToolBar title="查看" icon="new-icon-view" url="tScDepartmentController.do?goUpdate"
                         funname="detail"></t:dgToolBar>
            <t:dgToolBar title="复制" icon="new-icon-copy" url="tScDepartmentController.do?goUpdate"
            ...
            <t:dgToolBar title="打印" icon="new-icon-print"
                         funname="CreateFormPage('部门', '#tScDepartmentList')"></t:dgToolBar>
...
<script type="text/javascript">
    function dbClick(rowIndex, rowData) {
        url = 'tScDepartmentController.do?goUpdate' + '&load=detail&id=' + rowData.id;
        createdetailwindow('查看', url, null, null,"tab");
    }
    $(document).ready(function () {
        //  //添加datagrid 事件 onDblClickRow
//        $("#tScDepartmentList").datagrid({
//
...
//        });
        //给时间控件加上样式
            $("#tScDepartmentListtb").find("input[name='createDate']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function () {
                WdatePicker({dateFmt: 'yyyy-MM-dd'});
            });
            $("#tScDepartmentListtb").find("input[name='updateDate']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function () {
                WdatePicker({dateFmt: 'yyyy-MM-dd'});
            });
    });
