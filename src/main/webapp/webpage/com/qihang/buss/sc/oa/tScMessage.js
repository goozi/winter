$(function () {
	//用户选择触发事件
	//todo 后期确定ID要进行修改
	getUsers("8a8ab0b246dc81120146dc8180a20016");
});
...
function getUsers(type) {
	...
	var noticeId = $('#id').val();
	$('#userId').combotree({
		url: 'tScNoticeController.do?getUserTreeInfo&noticeId=' + noticeId+'&style=notice',
		multiple: true,
		onlyLeafCheck:false,
		cascadeCheck:true
	});
...
function commonUploadDefaultCallBack(url,key,name,extend,inputId){
	$("#"+inputId+"_href").attr('href',url).html('下载:'+name);
	$("#"+inputId).val(url);
}
