...
function checkForm(){
	var value = document.formobj.content.value.replace(/<[^>]+>/g,"").replace(/\s/g,"");//.replace(/(^\S*)|(\S*$)/g,"");
	if(value=="" || value == "&nbsp;"){
		tip("请输入合同内容!");
		return false;
	}
	if(value.length>1400000){
		tip("内容已超长度，臣妾做不到!");
		return false;
	}
}
...
function selectitemNameDialog() {
	...
	var url = 'tScOrganizationController.do?goSelectDialog&name=' + tempName + '&isDealer=1';
	var width = 1000;
	var height = 500;
	var title = "经销商";
	$.dialog({
		...
		button: [{
			...
			callback: function () {
				...
				$("input[name='itemIDName']").val(item.name);
