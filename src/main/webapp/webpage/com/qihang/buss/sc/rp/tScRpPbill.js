...
function resetTrNum(tableId) {
	...
	$tbody.find('>tr').each(function(i){
		...
		if(tableId.indexOf("1") > -1) {
			$("input[name='tScRpPbillentry1List[" + i + "].findex']").val(i + 1);
		}
	});
}
//通用弹出式文件上传
...
function orderListStockBlur(rowIndex,id,name){
	checkInput( $('input[name="tScRpPbillentry1List['+rowIndex+'].'+id+'"]'),$('input[name="tScRpPbillentry1List['+rowIndex+'].'+name+'"]'));
}
...
function selectItemNameDialog() {
	var itemName = $("#itemName").val();
	var url = 'tScSupplierController.do?goSelectDialog&name=' + itemName+"&isBillType=1";
	var width = 800;
	var height = 500;
	var title = "供应商";
	$.dialog({
...
function selectAccountDialog(index){
	var accountName = $("input[name='tScRpPbillentry1List["+index+"].accountName']").val();
	...
	$.dialog({
		...
		button: [{
			...
			callback: function () {
				...
				var nextTrLength = $('input[name="tScRpPbillentry1List[' + index + '].findex"]').parent().nextAll().length;//判断后面的行数
				var newLength = itemLength - nextTrLength - 1
				for (var j = 0; j < newLength; j++) {
					clickAddTScRpPbillentry1Btn(index);
				}
				$.each(itemList, function (i, item) {
					...
					$("input[name='tScRpPbillentry1List["+rowindex+"].accountId']").val(accountId);
					$("input[name='tScRpPbillentry1List["+rowindex+"].accountName']").val(accountName);
					setValOldIdAnOldName($("input[name='tScRpPbillentry1List["+rowindex+"].accountName']"),accountId,accountName);
				});
			},
			focus: true
		}, {
...
function selectBillDialog(){
	...
	var sonId = $("#sonId").val();
	if("51" ==  tranType) {
		url = "tPoOrderController.do?goSelectMainDialog&itemId=" + itemId + "&tranType=" + tranType +"&ids="+ids+"&ids2="+ids2+"&sonId="+sonId;
		url = encodeURI(url);
	} else if ("999" == tranType){
		title = "应收单"
		url = "tScPoStockbillController.do?goSelectMainDialog&itemid=" + itemId +"&ids="+ids+"&ids2="+ids2+"&sonId="+sonId;
		url = encodeURI(url);
	}
	var width = 800;
	var height = 500;
	...
	$.dialog({
		...
		button: [{
			...
			callback: function () {
				...
				var length = $("#add_tScRpPbillentry2_table").find(">tr").length;
				length--;
				var id = $("input[name='tScRpPbillentry2List["+length+"].classIdSrc']").val();
				var dataLength = itemList.length;
				if(!id){
					dataLength--;
				}
				for(var k=0 ; k < dataLength ; k++){
					clickAddTScRpPbillentry2Btn(length)
				}
				if(id){
					length++;
				}
				...
				if("51" == tranType) {
					for (var i = 0; i < itemList.length; i++) {
						...
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].findex']").val(i + 1);
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].idSrc']").val(idSrc);
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].classIdSrc']").val(classIdSrc);
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].billnoSrc']").val(billnoSrc);
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].dateSrc']").val(dateSrc);
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].billAmount']").val(billAmount);
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].billAmount']").trigger("input");
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].billCheckAmount']").val(billCheckAmount);
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].billCheckAmount']").trigger("input");
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].billUnCheckAmount']").val(billUnCheckAmount);
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].billUnCheckAmount']").trigger("input");
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].classIdName']").val("采购订单");
					...
					for (var i = 0; i < itemList.length; i++) {
						...
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].billUnCheckAmount']").val(billUnCheckAmount);
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].billUnCheckAmount']").trigger("input");
						$("input[name='tScRpPbillentry2List[" + rowIndex + "].classIdName']").val(classIdName);
						if("53" != classIdSrc) {
							allUnCheckAmount = parseFloat(allUnCheckAmount) + parseFloat(billUnCheckAmount);
						}
					}
...
function checkAmount(index){
	var amount = $("input[name='tScRpPbillentry2List["+index+"].amount']").val();
	var tranType = $("input[name='tScRpPbillentry2List["+index+"].classIdSrc']").val();//单据类型
	if(parseFloat(amount) < 0){
		amount = -1 * parseFloat(amount);
	}
	var billUnCheckAmount = $("input[name='tScRpPbillentry2List["+index+"].billUnCheckAmount']").val();
	...
	if(parseFloat(amount) > parseFloat(billUnCheckAmount)){
		...
		$("input[name='tScRpPbillentry2List["+index+"].amount']").val(billUnCheckAmount)
		setAllAmount2();
		return;
	}
	if("53" == tranType){
	...
	//$("input[name='tScRpPbillentry2List["+index+"].amount']").val(amount)
	setAllAmount2();
}
...
function setAllAmount(){
	var CFG_MONEY = $("#CFG_MONEY").val();
	$tbody = $("#tScRpPbillentry1_table");
	var length = $tbody[0].rows.length;
	var allAmount = 0;
	for(var i=0 ; i<length ; i++){
		var amount = $("input[name='tScRpPbillentry1List[" + i + "].amount']").val();//金额
		if(!amount){
			amount = 0;
		}

		allAmount += parseFloat(amount);
		$("input[name='tScRpPbillentry1List[" + i + "].amount']").val(parseFloat(amount).toFixed(CFG_MONEY))
	}
...
function setAllAmount2(){
	var CFG_MONEY = $("#CFG_MONEY").val();
	$tbody = $("#tScRpPbillentry2_table");
	var length = $tbody[0].rows.length;
	var allAmount = 0;
	var allAmount_ = 0;
	for(var i=0 ; i<length ; i++){
		debugger;
		var tranType = $("input[name='tScRpPbillentry2List["+i+"].classIdSrc']").val();//源单类型
		var amount = $("input[name='tScRpPbillentry2List[" + i + "].amount']").val();//金额
		if(!amount){
			amount = 0;
		}
		if("53" != tranType){
			var unCheckAmount = $("input[name='tScRpPbillentry2List["+i+"].billUnCheckAmount']").val();//未执行金额
...
function fentang(){
	...
	$tbody = $("#tScRpPbillentry2_table");
	var length = $tbody[0].rows.length;
	var id = $("input[name='tScRpPbillentry2List["+(length-1)+"].classIdSrc']").val();
	if(!id){
		tip("请对应收表进行选单操作");
		$("#chooseType2").val("");
		return ;
	...
	if("0001" == chooseType){
		for(var j=0 ; j< length ; j++){
			...
			var unCheckAmount = $("input[name='tScRpPbillentry2List[" + j + "].billUnCheckAmount']").val();
			if ("53" == tranType) {
				allAmount = parseFloat(allAmount) - parseFloat(unCheckAmount);
				$("input[name='tScRpPbillentry2List[" + j + "].amount']").val(unCheckAmount);
			}
		}
		for(var i=0 ; i<length ; i++){
			var tranType = $("input[name='tScRpPbillentry2List["+i+"].classIdSrc']").val();
			var unCheckAmount = $("input[name='tScRpPbillentry2List["+i+"].billUnCheckAmount']").val();
			if("53" != tranType){
				if (parseFloat(allAmount) > parseFloat(unCheckAmount)) {
					$("input[name='tScRpPbillentry2List[" + i + "].amount']").val(unCheckAmount);
					$("input[name='tScRpPbillentry2List[" + i + "].amount']").trigger("change");
					allAmount = (parseFloat(allAmount) - parseFloat(unCheckAmount)).toFixed(CFG_MONEY);
				} else {
					$("input[name='tScRpPbillentry2List[" + i + "].amount']").val(allAmount);
					$("input[name='tScRpPbillentry2List[" + i + "].amount']").trigger("change");
					allAmount = (parseFloat(allAmount) - parseFloat(allAmount)).toFixed(CFG_MONEY);
				}
			} else {
				$("input[name='tScRpPbillentry2List[" + i + "].amount']").trigger("change");
			}
		...
		for(var i=0 ; i<length ; i++) {
			...
			if("53" != tranType) {
				//if (parseFloat(allAmount) > parseFloat(unCheckAmount)) {
				var amount = (parseFloat(unCheckAmount) / parseFloat(allUnCheckAmount) * parseFloat(allAmount)).toFixed(CFG_MONEY);

				$("input[name='tScRpPbillentry2List[" + i + "].amount']").val(amount);
				$("input[name='tScRpPbillentry2List[" + i + "].amount']").trigger("change");
				//allAmount = parseFloat(allAmount) - parseFloat(amount);
				//} else {
				//	$("input[name='tScRpPbillentry2List[" + i + "].amount']").val(allAmount);
