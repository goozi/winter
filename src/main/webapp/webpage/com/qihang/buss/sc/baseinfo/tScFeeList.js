...
function  refreshRightData(node){
    ...
    $("#tScFeeList").datagrid("load",{name:name,number:number,
...
function goAdd(){
    ...
    var parentNo = info[1];
    add('录入','tScFeeController.do?goAdd&parentID='+nodeTree.id+'&parentNo='+parentNo,'tScFeeList',null,null,'tab')
...
function goUpdate(){
    ...
    if($("#tScFeeList").datagrid("getSelected").disabled=='1'){
    ...
    update('编辑','tScFeeController.do?goUpdate','tScFeeList',null,null,'tab');
