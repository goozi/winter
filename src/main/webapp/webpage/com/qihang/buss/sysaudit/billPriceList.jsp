...
<div class="easyui-layout" fit="true">
	<div data-options="region:'west',title:'单据类型',split:true" style="width:200px;">
		<ul id="tree" class="easyui-tree" data-options="url:'auditController.do?getMenuTree&isPrice=1'"></ul>
	</div>

	<div region="center" style="padding:1px;">
		<t:datagrid name="tScOrganizationList" checkbox="false" checkOnSelect="true" pagination="false" fitColumns="false" title="单价明细"
					actionUrl="auditController.do?datagrid&isPrice=1" idField="id" fit="true"
					queryMode="group">
			...
			<t:dgCol title="单价字段" field="priceField"  dictionary="SC_BillPriceType" queryMode="single"   width="120"></t:dgCol>
			<t:dgToolBar title="编辑" icon="icon-add"
	...
	$(document).ready(function () {
		...
		$("#tScOrganizationList").datagrid({
			...
			onDblClickRow: function (rowIndex, rowData) {
				url = 'tSBillInfoController.do?goUpdatePrice' + '&load=detail&infoId=' + rowData.id;
				createdetailwindow('查看', url, 250, 250);
...
//		}
		...
		var gridSelect = $("#tScOrganizationList").datagrid("getSelected");
		if(null != gridSelect){
			infoId = gridSelect.id;
		}
		var url = "tSBillInfoController.do?goUpdatePrice&infoId="+infoId;
		createwindow("编辑", url, 250, 250);
	}
